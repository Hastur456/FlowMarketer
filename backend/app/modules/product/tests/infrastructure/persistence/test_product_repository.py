...
async def test_create_returns_domain_product(
    ...
    assert created.gallery_urls == [
        "https://example.com/products/1.jpg"
    ]


async def test_find_by_id_returns_product(
    product_repository: SqlAlchemyProductRepository,
    db_session: AsyncSession,
    category,
):
    ...
    found = await product_repository.find_by_id(
        created.id,
        session=db_session,
    )
...
async def test_update_changes_product_fields(
    product_repository: SqlAlchemyProductRepository,
    db_session: AsyncSession,
    category,
):
...
async def test_delete_returns_deleted_product(
    product_repository: SqlAlchemyProductRepository,
    db_session: AsyncSession,
    category,
):
    ...
    deleted = await product_repository.delete(
        created.id,
        session=db_session,
    )
...
async def test_find_active_returns_only_active_products(
    ...
    result = await product_repository.find_active(
        session=db_session,
    )
...
async def test_find_featured_returns_only_active_featured_products(
    ...
    result = await product_repository.find_featured(
        session=db_session,
    )
